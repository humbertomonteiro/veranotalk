...
//                 couponComissioned={coupon.isCommissioned}
//                 participants={participants}
//               />
//               <PaymentInfo />
//             </div>
...
const COMMISSIONED_PRICES = [
  { minQuantity: 5, price: 329.9 },
  { minQuantity: 2, price: 379.9 },
];

// Preço base para cupom comissionado com 2+ ingressos (sem desconto adicional)
function getCommissionedPrice(tickets: number): number | null {
  return (
    COMMISSIONED_PRICES.find((t) => tickets >= t.minQuantity)?.price ?? null
  );
}
...
export default function Checkout() {
  ...
  // Preço padrão sem cupom (lógica original preservada)
  const getBasePrice = (tickets: number) => {
    if (isGroupTicket) return ticketGroup;
    if (isDoubleTicket) return ticketDouble;
    if (tickets >= 5) return ticketGroup;
  ...
  // ─── Flags derivadas do cupom ──────────────────────────────────────────────

  const isCommissionedCoupon = !!coupon?.isCommissioned;
  const hasCouponApplied = Object.keys(coupon).length > 0;

  // Cupons não comissionados: só 1 ingresso. Comissionados: qualquer quantidade.
  // const couponsAllowed = fullTickets === 1 || isCommissionedCoupon;

  // ─── Cálculo de preço e desconto ──────────────────────────────────────────

  const commissionedPriceOverride = isCommissionedCoupon
    ? getCommissionedPrice(fullTickets)
    : null;

  // Se cupom comissionado e 2+ ingressos → usa tabela especial, sem desconto adicional
  // Se cupom comissionado e 1 ingresso   → preço padrão (499) com desconto do cupom
  // Qualquer outro caso                  → preço padrão
  const unitPrice =
    commissionedPriceOverride !== null
      ? commissionedPriceOverride
      : getBasePrice(fullTickets);

  const subtotal = unitPrice * fullTickets;

  // Desconto só aplica quando: há cupom E (1 ingresso OU cupom não comissionado)
  const discountApplies =
    hasCouponApplied && (fullTickets === 1 || !isCommissionedCoupon);
  ...
  useEffect(() => {
    setBasePrice(getBasePrice(fullTickets));
    console.log(basePrice);
  }, [fullTickets, id]);

  useEffect(() => {
    // Se o cupom não é comissionado e o usuário passou para 2+ ingressos, limpa
    if (!isCommissionedCoupon && fullTickets > 1 && hasCouponApplied) {
      setCouponCode("");
      setCoupon({});
      setDiscountValueCoupon(0);
    }
  ...
  useEffect(() => {
    if (id && id !== "1" && id !== "2" && id !== "3") {
      // URL com código de cupom direto
      setCouponCode(id.toLowerCase());
      handleApplyCoupon(id.toLowerCase(), true);
    }
  }, [id]);
  ...
  const handleIncrement = () => setFullTickets((prev) => prev + 1);

  const handleDecrement = () => {
    if (fullTickets <= minTickets) {
      toast.error(
        `Mínimo de ${minTickets} ingresso${minTickets > 1 ? "s" : ""} para este lote`,
      );
      return;
    }
    setFullTickets((prev) => prev - 1);
  };
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value !== n) {
      toast.error(
        `Mínimo de ${minTickets} ingresso${minTickets > 1 ? "s" : ""} e máximo de 50`,
      );
    }
    setFullTickets(value);
  };
  ...
  const handleCouponChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (fullTickets > 1 && !isCommissionedCoupon) {
      toast.error("Cupons são permitidos apenas para 1 ingresso");
      return;
    }
    setCouponCode(e.target.value);
  ...
  const handleApplyCoupon = async (
    ...
    try {
      ...
      // Valida: cupom não comissionado só pode ser aplicado com 1 ingresso
      if (!data.coupon.isCommissioned && fullTickets > 1) {
        if (!silent) toast.error("Este cupom é válido apenas para 1 ingresso");
        return;
      }
          ...
          <div className={styles.content}>
            ...
            <div className={styles.visualContent}>
              <SummaryCard
                ...
                discount={discountAmount}
                couponCode={couponCode}
                couponComissioned={isCommissionedCoupon}
                participants={participants}
              />
              <PaymentInfo />
            </div>
